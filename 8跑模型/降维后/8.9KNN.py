...
# 定义网格参数（适用于 KNN）
param_grid = {
    'n_neighbors': [3, 5, 7, 9, 11],       # 邻居数量 K
    'weights': ['uniform', 'distance'],     # 权重方式：均匀或基于距离
    'metric': ['euclidean', 'manhattan', 'minkowski'],  # 距离度量
    'p': [1, 2]                             # Minkowski 距离的参数（1 为曼哈顿，2 为欧几里得）
}
...
# 特征重要性（KNN 不提供直接的特征重要性）
print("\n注意：KNeighborsClassifier 不提供直接的特征重要性。")
