...
def get_features(data):
    ...
    # 处理技术知识元向量列
    tech_vector_features = preprocess_vector_column(data['reduced_应用知识元向量'], 'reduced_应用知识元向量')
    ...
    X = pd.concat([X_numeric, tech_vector_df], axis=1)
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    # 定义模型和参数网格
    lr = LogisticRegression(random_state=100, max_iter=1000)  # 增加max_iter以确保收敛
...
def main():
    # 输入文件路径
    # positive_file = r'C:\Users\hhhxj\Desktop\专利数据\9数据清洗\传统＋技术网络指标(大于10年)_cleaned.xlsx'
    # negative_file = r'C:\Users\hhhxj\Desktop\专利数据\9数据清洗\传统＋技术网络指标(小于等于10年)_cleaned.xlsx'
